...
class Qwen3_5ForCausalLMMTP(nn.Module):
    ...
    def __init__(
        ...
        # Compressed-tensors targets can also keep the embedded MTP branch in
        # BF16. Build the draft module without the target's NVFP4 quantizer when
        # every serialized mtp.* tensor is explicitly excluded. Otherwise the
        # constructor allocates separate unquantized projections while the
        # loader expects fused gate_up/qkv parameters, silently skips the BF16
        # draft weights, and produces zero speculative acceptance.
        if quant_config and quant_config.get_name() == "compressed_tensors":
            ignore_layers = getattr(quant_config, "ignore", [])
            if any(
                isinstance(layer, str) and layer.startswith("mtp.")
